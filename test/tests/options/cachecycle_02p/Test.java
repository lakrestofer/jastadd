// Test that the cacheCycle option has no effect in concurrent mode.
// https://bitbucket.org/jastadd/jastadd2/issues/295/cachecycle-option-is-not-working
// .grammar: { A; }
// .options: cacheCycle tracing=compute concurrent
// .source: 1.8
import java.util.*;
...
public class Test {
  ...
  public static void main(String[] args) {
    ...
    // Set up compute logging in order to check which attributes were logged.
    ComputeLog log = new ComputeLog();
    a.trace().setReceiver(log);

    // Note: x() uses y(), so y() should already be memoized.
    a.y();
    testThat(log.log).containsExactly("A.y()");
  }
}
