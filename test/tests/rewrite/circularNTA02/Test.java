// Tests evaluation of rewrite using circular NTA algorithm, where rewritten
// value uses inherited attributes.
...
public class Test {
  ...
  public static void main(String[] args) {
    Program p = new Program(new B(), new D());

    EvalTracker tracker = new EvalTracker();
    p.trace().setReceiver(tracker);

    testEqual(-1, p.getA().value());
    testEqual(1, p.getA().a());
    testEqual(1, p.getA().b());
    testEqual(1, p.getA().c());
    testEqual(2, p.getD().c());
    p.flushTreeCache(); // Flushes the rewrite cnta cache.
